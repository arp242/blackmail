...
type Client struct {
	...
	CommandTimeout    time.Duration     // Wait for command responses (includes 3xx reply to DATA).
	SubmissionTimeout time.Duration     // Wait for responses after final dot.
	DebugWriter       io.Writer         // Logger for all network activity.
}

// New returns a new Client using an existing connection and host as a
// server name to be used when authenticating.
func New(conn net.Conn, serverName string) *Client {
	c := &Client{
		localName: "localhost",
		// As recommended by RFC 5321. For DATA 3xx command reply it recommends
		// a shorter timeout, but we don't bother differentiating this.
	...
	c.setConn(conn)
	c.serverName, _, _ = net.SplitHostPort(serverName)
	return c
}
...
func DialTLS(addr string, tlsConfig *tls.Config) (*Client, error) {
	tlsDialer := tls.Dialer{NetDialer: &defaultDialer, Config: tlsConfig}
...
func (c *Client) setConn(conn net.Conn) {
	...
	c.text = textproto.NewConn(struct {
		...
		Reader: io.TeeReader(r, clientDebugWriter{[]byte("SERVER  "), c}),
		Writer: io.MultiWriter(conn, clientDebugWriter{[]byte("CLIENT  "), c}),
		Closer: conn,
	})
}
...
func (c *Client) StartTLS(config *tls.Config) error {
	...
	if config.ServerName == "" && c.serverName != "" {
		config = config.Clone() // Make a copy to avoid polluting argument
...
func (c *Client) Mail(from string, opts *MailOptions) error {
	...
	sb.Grow(2048) // A high enough power of 2 than 510+14+26+11+9+9+39+500
	fmt.Fprintf(&sb, "MAIL FROM:<%s>", from)
	if _, ok := c.ext["8BITMIME"]; ok {
		sb.WriteString(" BODY=8BITMIME")
	}
	// TODO: disabled for now; need to check, but can just calculate automatically?
...
// SendMail will use an existing connection to send an email from address from,
// to addresses to, with message data.
//
...
//
// The data parameter should be an RFC 822-style email with headers first, a
// blank line, and then the message body. The lines of data should be CRLF
// terminated. The data headers should usually include fields such as "From",
...
// data headers.
func (c *Client) SendMail(from string, to []string, data io.Reader) error {
	var err error
...
func (cdw clientDebugWriter) Write(b []byte) (int, error) {
	...
	for {
		...
		_, err := cdw.c.DebugWriter.Write(cdw.p)
		if err != nil {
			return n, err
		}
		nn, err := cdw.c.DebugWriter.Write(b[:i+2])
		if err != nil {
			return n + nn, err
		}
		n += nn
